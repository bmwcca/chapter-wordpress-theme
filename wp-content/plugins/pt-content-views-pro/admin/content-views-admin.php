	...
	public function filter_view_type_settings_grid( $args ) {
		...
		$forcemask = array(
			...
			'params'		 => array(
				array(
					'type'		 => 'checkbox',
					'name'		 => 'force-mask',
					'options'	 => PT_CV_Values::yes_no( 'yes', __( 'Put mask of fields in front of thumbnail', 'content-views-pro' ) ),
					'std'		 => '',
					'popover'	 => sprintf( "<img src='%s'>", plugins_url( 'assets/images/popover/force-mask.png', __FILE__ ) ),
				),
			),
		);
		...
		$nospace = array(
			...
			'params'		 => array(
				array(
					'type'		 => 'checkbox',
					'name'		 => 'no-space',
					'options'	 => PT_CV_Values::yes_no( 'yes', __( 'Remove space between posts', 'content-views-pro' ) ),
					'std'		 => '',
					'popover'	 => sprintf( "<img src='%s'>", plugins_url( 'assets/images/popover/no-space.png', __FILE__ ) ),
				),
			),
			'dependence'	 => array( 'force-mask', 'yes' ),
		);
		...
		$border = array(
			...
			'params'		 => array(
				array(
					...
					'std'		 => '',
					'popover'	 => sprintf( "<img src='%s'>", plugins_url( 'assets/images/popover/post-border.png', __FILE__ ) ),
				),
			),
		);
		...
		$border_desc = array(
			...
			'params'		 => array(
				array(
					'type'		 => 'html',
					'content'	 => sprintf( '<p class="text-muted" style="margin-top: -10px">%s</p>', __( 'If border is discontinuous, please check above setting', 'content-views-pro' ) ),
				),
			),
		...
		$args = array_merge(
			...
			array(
				...
				'params'	 => array(
					array(
						'type'		 => 'html',
						'content'	 => sprintf( '<p class="text-muted" style="padding-left:5px;color:#ff5a5f">%s</p>', __( 'It takes time and resources to generating new images. Please limit number of posts to show, or enable pagination.', 'content-views-pro' ) ) . sprintf( '<p class="text-muted" style="padding-left: 5px;">%s</p>', __( 'If output images are not same size, please decrease above values (because it is bigger than smallest image)', 'content-views-pro' ) ),
					),
				),
				'dependence' => array( $prefix . 'thumbnail-size', PT_CV_PREFIX . 'custom' ),
			),
