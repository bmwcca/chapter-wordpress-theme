...
if ( !class_exists( 'PT_CV_Html_Pro' ) ) {
	...
	class PT_CV_Html_Pro {
		...
		static function view_styles( $view_styles ) {
			...
			$hover_enable	 = isset( $animation[ 'content-hover' ] );
			if ( $hover_enable ) {
		...
		static function custom_fields_html( $object, $is_post = true ) {
			...
			if ( $custom_fields_st && !empty( $custom_fields_st[ 'list' ] ) ) {
				...
				$wanted_keys = $is_post ? array_intersect( $list, array_keys( $metadata ) ) : $list;
				...
				foreach ( $wanted_keys as $idx => $key ) {
					...
					// If field value is empty, extract it from Metadata
					...
					// Better field output
					if ( $field_type === 'text' ) {
						$field_value = PT_CV_CustomField::display_output( (array) $field_value, $use_oembed );
					}

					// Date value
					if ( !empty( $ctf_date_format ) ) {
						$try_convert = self::date_convert( $field_value );
					...
					$value .= sprintf( '<div class="%s">%s</div>', PT_CV_PREFIX . 'ctf-value', apply_filters( PT_CV_PREFIX_ . 'ctf_value', $field_value, $key, $object ) );
