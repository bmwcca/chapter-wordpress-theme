...
if ( !class_exists( 'PT_CV_Hooks_Pro' ) ) {
	...
	class PT_CV_Hooks_Pro {
		...
		public static function filter_post_status( $args ) {
			...
			if ( PT_CV_Functions::setting_value( PT_CV_PREFIX . 'post_date_value' ) === 'from_today' ) {
		...
		public static function filter_field_thumbnail_not_found( $args, $post, $dimensions, $gargs ) {
			...
			if ( !empty( $first_img ) ) {
				...
				$found_image	 = sprintf( '<img src="%s" class="%s" %s/>', esc_attr( $first_img ), esc_attr( $gargs[ 'class' ] . ' cvp-substitute' ), $width_height );
			}
		...
		public static function embed_video( $media_url, $dimensions ) {
			...
			if ( !empty( $media_url ) ) {
				...
				if ( preg_match( '(youtube\.com|youtu\.be)', $media_url ) ) {
					...
					if ( !empty( $matches[ 2 ] ) ) {
						if ( apply_filters( PT_CV_PREFIX_ . 'thumbnail_youtube', false ) ) {
							$args = sprintf( '<div class="cvp-youtube" id="%s" style="width: %spx; height: %spx"></div>', $matches[ 2 ], $dimensions[ 0 ], $dimensions[ 0 ] * 2 / 3 );
						} else {
		...
		public static function filter_field_href_class( $args, $oargs ) {
			...
			/**
			 * @since 3.9.5: Fix "Maximum call stack size exceeded" of Lightbox when no Featured image and first image found
			 */
			if ( isset( $post->cvp_full_image ) && empty( $post->cvp_full_image ) ) {
				$args = explode( ' ', str_replace( PT_CV_PREFIX . 'lightbox-image', PT_CV_PREFIX . 'lightbox', implode( ' ', $args ) ) );
			}
		...
		public static function filter_field_href( $args, $post ) {
			...
			if ( isset( $other_settings[ 'open-in' ] ) ) {
				if ( $other_settings[ 'open-in' ] === PT_CV_PREFIX . 'none' ) {
					...
					if ( isset( $post->cvp_full_image ) ) {
						$full_image = !empty( $post->cvp_full_image ) ? $post->cvp_full_image : $full_image;
					} else {
						...
						if ( !self::_is_attachment( $post ) ) {
							if ( has_post_thumbnail( $post->ID ) ) {
								...
								$full_image = self::get_inside_image( $post, 'full', $post->content );
							}
						} else {
						...
						$post->cvp_full_image = $full_image;
					}
		 ...
		 *
		 * @deprecated since version 3.9.4 (CV 1.8.8)
		 *
		 * @param string $args  The readmore text
		 * @param array  $fargs The settings of Content
		 */
		...
		public static function filter_page_attr( $args, $view_type, $content_items ) {
			...
			if ( PT_CV_Functions::get_global_variable( 'enable_shuffle_filter' ) ) {
				...
				if ( $outside_operator = PT_CV_Functions::setting_value( PT_CV_PREFIX . 'taxonomy-filter-cross-operator' ) ) {
					$args .= sprintf( ' data-sfop="%s"', $outside_operator );
				}
			}

			if ( PT_CV_Functions::setting_value( PT_CV_PREFIX . 'grid-same-height' ) ) {
				$args .= ' data-cvct="' . PT_CV_Functions::setting_value( PT_CV_PREFIX . 'resp-tablet-number-columns' ) . '"';
				$args .= ' data-cvcm="' . PT_CV_Functions::setting_value( PT_CV_PREFIX . 'resp-number-columns' ) . '"';
			}
		...
		public static function filter_query_parameters( $args ) {
			...
			// Include current posts (keep it for old Views)
			...
			if ( defined( 'PT_CV_DOING_PAGINATION' ) && isset( $sf_custom_data[ 'sf_taxo' ], $sf_custom_data[ 'sf_pids' ] ) ) {
				...
				if ( is_array( $view_tt ) ) {
					...
					if ( $subtax ) {
						...
						$args[ 'posts_per_page' ] = $sf_settings[ 'show-all' ] ? 1000 : (int) PT_CV_Functions::setting_value( PT_CV_PREFIX . 'pagination-items-per-page' ); // -1 will ignore offset value
					}
						...
						foreach ( $dargs[ 'fields' ] as $field ) {
							...
							switch ( $field ) {
								case 'thumbnail':
									...
									if ( apply_filters( PT_CV_PREFIX_ . 'term_thumbnail', $term_img, $term ) ) {
		...
		public static function filter_view_class( $args ) {
			...
			$fields = array( 'force-mask', 'no-space', 'post-border' );
			foreach ( $fields as $field ) {
				if ( PT_CV_Functions::setting_value( PT_CV_PREFIX . $field ) ) {
					$args[] = PT_CV_PREFIX . $field;
				}
			}
		...
		private static function _content_items_ads( $args, $view_type ) {
			if ( defined( 'PT_CV_VIEW_REUSE' ) || ( defined( 'PT_CV_VIEW_OVERWRITE' ) && !PT_CV_Functions::get_option_value( 'show_ads_anywhere' ) ) ) {
				return $args;
			}
